...
class rdc
{
  ...
  private $client;
  private $tempfilename = "MyRealtor-k-Maritn.vcf";
  // debuging tools
  public $s="";
  public function pr($ss)
  {
      //$this->s.=$ss."\r\n";
      //echo "$ss"."\r\n";
  }
  ...
  {
      file_put_contents("output.txt",$this->s);
  ...
  function __construct()
  {
    ...
    public function run_email_loop()
    {
        ...
        for ($i = 0; $i < 10; $i++) {
            ...
            $delay           = $milliseconds - $oldmilliseconds;
            $oldmilliseconds = $milliseconds;
            $this->pr("delay : " . $delay . " milli");
            sleep(4);
        } //end of loop that is done 30 timese
    }

    private function read_and_process_email($lead_from)
    {
        $this->pr("read and process eamil ==============");
        //file_put_contents("output.txt","begin of read and process");
        ...
        if ($emails) {
            ...
            foreach ($emails as $mail) {
                ...
                if (strpos($headerInfo->subject, $lead_from["subject_search"]) !== false) {
                    ...
                    $city                                           = end($carray);
                    $property_value                                 = preg_replace('/[^0-9]/', '', str_replace('$', '', trim($this->get_string_between($email_body, 'Basic Property Attributes:', 'Bed'))));
    ...
    private function getlastLeadTimeSpan($n)
    {
        $sth = $this->db_con->prepare("SELECT *,TIMESTAMPDIFF(MINUTE,`created_date`,NOW())as timediff FROM `leads` WHERE Agent like '" . $n . "' order by `created_date`-Now() limit 1 ");
        $sth->execute();
        $rows = $sth->fetch(PDO::FETCH_ASSOC);
        $this->pr($sth->rowCount());
        // var_dump($rows);
        if ($sth->rowCount() == 1) {
            return $rows['timediff'];
        } else {
            return false;
        }
    }
    ...
    {
        $kellylast = $this->getlastLeadTimeSpan("kelly");
        $cindylast = $this->getlastLeadTimeSpan("cindy");
        $this->pr("kellylast:" . $kellylast);
        $this->pr("cindylast:" . $cindylast);
        $agent = "none";
        $this->pr("parse action =-=-=-=-=-=-=-emailconfig=$this->email_config[username]");
        if ($this->email_config["username"] == "email@rdcbravo.com") {
            $use_domain_set = $this->domain_set["Bravo"];
            $this->pr("set domain set to bravo");
        } else if ($this->email_config["username"] == "email@rdcalpha.com") {
            $use_domain_set = $this->domain_set["Alpha"];
            $this->pr("set domain set to alpha");
        }
        // $this->testvcard("somezp");//,$mobile);
        ...
        $this->pr("datatext==$data_text");
        $lastid  = $this->saveRecordToDB($data_array);
        $link    = "http://rdccharlie.com/basecrm/leadinfo.php?id=" . $lastid;
        $link1   = "http://myrprreport.com/AVM.php?address=" . urlencode($data_array['address1']);
        $mobile  = preg_replace('/[^0-9]/', '', $data_array['mobile']);
        $Arizona = (int) date('H');
        //  $this->updateRecord($lastid, "something");
        $specificzips = array(
            "85018",
            "85253"
        );
        if ($Arizona < 21 and $Arizona >= 8) {
            if (strpos($data_array['custom_fields']['Inquiry Comment'], 'lease') == false or strpos($data_array['custom_fields']['Inquiry Comment'], 'rent') == false) {
                ...
                if ($use_domain_set["app_name"] == "Bravo" && (in_array($data_array['zip'], $specificzips))) {
                    $agent = "charlie";
                } else {
                    if ($property_value >= 250000) {
                        $agent = "main";
                    } else if ($property_value < 250000 && $property_value > 200000) {
                        $agent = "cindy";
                    } else {
                        if ($kellylast > 60)
                            $agent = "kelly";
                        else if ($kellylast > 60)
                            $agent = "kelly";
                        else if ($kellylast > $cindylast)
                            $agent = "kelly";
                        else
                            $agent = "cindy";
                    }
        ...
        if ($agent == "charlie") {
            $this->pr("2---------,dataarray[zip]=" . $data_array['zip'] . ",postrulecode:" . $lead_from["post_rule_code"]);
            $use_domain_set = $this->domain_set["Charlie"];
            $this->pr("userdomain_set[to]:" . $use_domain_set["to"]);
            $this->pr("-------------------------------");
            $this->pr("21");
            $this->SendSMS($data_array['mobile'], "+14805258426", $link1, "", "");
            $this->pr("22");
            $this->twilio_make_call($use_domain_set["from"], $use_domain_set["to"], $data_array['mobile']);
            $this->testvcard($data_array['zip'], $data_array['mobile']);
            $this->SendSMS($use_domain_set["from"], "+14805258426", $link, "", "");
            $this->pr("23");
            $this->pr("24");
            // wrong  $this->twilio_make_call($use_domain_set["from"], $use_domain_set["alt_number"], $data_array['mobile']);
        }
        if ($agent == "main") {
            $this->pr("1---------,dataarray[zip]=" . $data_array['zip'] . ",postrulecode:" . $lead_from["post_rule_code"]);
            //$use_domain_set = $this->domain_set["Main"];
            $this->pr("userdomain_set[to]:" . $use_domain_set["to"]);
            $this->pr("-------------------------------");
            $this->testvcard($data_array['zip'], $data_array['mobile']);
            $this->twilio_make_call($use_domain_set["from"], "+14805256665", $data_array['mobile']);
            $this->SendSMS("14805256665", "+14805258426", $link, "", "");
            $this->pr("11");
            $this->SendSMS($data_array['mobile'], "+14805258426", $link1, "", "");
            // wrong  $this->twilio_make_call($use_domain_set["from"], $use_domain_set["alt_number"], $data_array['mobile']);
        }
        if ($agent == "kelly") {
            $this->pr("4---------");
            $this->pr("dataarray[zip]=" . $data_array['zip'] . ",postrulecode:" . $lead_from["post_rule_code"]);
            $this->pr("userdomain_set[to]:" . $use_domain_set["to"]);
            $this->pr("-------------------------------");
            // $this->testvcardkelly($data_array['zip']);//,$mobile);
            $this->pr(" from=$use_domain_set[from],alt_number= $use_domain_set[alt_number],dataarray-mobile= $data_array[mobile]);");
            $this->send_email("kmichaelsre@gmail.com", "New Lead " . $use_domain_set["app_name"], $data_text);
            $this->pr("41");
            $this->pr("42");
            $this->pr("43");
            $this->testvcardKelly($data_array['zip'], $data_array['mobile']); //,$mobile);
            $this->pr("44");
            $this->twilio_make_call("+16023130211", "+16029260047", $data_array['mobile']);
            $this->pr("45");
            $this->SendSMS("+16029260047", "+14805258426", $link, "", "");
            $this->SendSMS($data_array['mobile'], "+14805258426", $link1, "", "");
            //wrong  $this->twilio_make_call($use_domain_set["from"], $use_domain_set["alt_number"], $data_array['mobile']);
        }
        if ($agent == "kelly") {
            ...
            $this->pr("userdomain_set[to]:" . $use_domain_set["to"]);
            $this->pr("-------------------------------");
            //$this->SendSMS("+16023631448", "+14805258426",$link,"","");
            //$this->SendSMS("+16023631448", "+14805258426",$link1,"","");
            $this->pr("51");
            $this->send_email("kmichaelsre@gmail.com", "New Lead " . $use_domain_set["app_name"], $data_text);
            $this->pr("52");
            $this->testvcardkelly($data_array['zip'], $mobile); //$data_array['mobile']);//,);
            $this->pr("53");
            $this->twilio_make_call("+16023131570", "16023965471", $data_array['mobile']);
            $this->pr("54");
            $this->SendSMS("+16023965471", "+14805258426", $link, "", "");
            $this->pr("55");
            $this->pr("52");
            $this->SendSMS($data_array['mobile'], "+14805258426", $link1, "", "");
        ...
        $this->pr("end agent : " . $agent);
        $this->updateRecord($lastid, $agent);
    }

    private function twilio_make_call($from, $to, $lead_number)
    {
        $this->pr("twilio make call" . $from . "," . $to . "," . $lead_number);
        ...
        try {
            $call = $this->client->calls->create($to, $from, array(
                "url" => $this->base_domian_path . $this->twilio_gather_file
            ));
            if ($this->debug) {
    ...
    //----------------<codenassor>
    public function testvcard($zip, $tonumber)
    {
        //$zip="somezip";
        $currentpath = "http://rdccharlie.com/basecrm/";
        // Your Account Sid and Auth Token from twilio.com/user/accouAnt
        $this->SendVCF($tonumber, "+14805258426", "K Martin - MY REALTOR", $currentpath . $this->tempfilename, $zip);
        $this->SendSMS($tonumber, "+14805258426", "Hi, I'm \"K\" Martin a 17yr professional real estate agent. Please save my contact info card I just sent you! I save my clients 1000's of dollars every deal. No Games! Call or text when ready.", "http://arabfacts.rf.gd/twilio/" . $this->tempfilename, $zip);
    }

    // public function testvcardCindy($zip, $tonumber)
    // {
    //     //$zip="somezip";
    //     $currentpath = "http://rdccharlie.com/basecrm/";
    //     // Your Account Sid and Auth Token from twilio.com/user/accouAnt
    //     $this->SendVCFKelly($tonumber, "+14805258426", "K Martin - MY REALTOR", $currentpath . $this->tempfilename, $zip);
    //     $this->SendSMS($tonumber, "+14805258426", "Hi, I'm \"K\" Martin a 17yr professional real estate agent. Please save my contact info card I just sent you! I save my clients 1000's of dollars every deal. No Games! Call or text when ready.", "http://arabfacts.rf.gd/twilio/" . $this->tempfilename, $zip);
    // }

    public function testvcardkelly($zip, $tonumber)
    {
        //$zip="somezip";
        //----------------<codenassor>
        $currentpath = "http://rdccharlie.com/basecrm/";
        // Your Account Sid and Auth Token from twilio.com/user/accouAnt
        $this->SendVCFkelly($tonumber, "+14805258426", "Kelly Michaels - MY REALTOR", $currentpath . $this->tempfilenamekelly, $zip);
        $this->SendSMS($tonumber, "+14805258426", "Hi, I'm \"K\" Martin a 17yr professional real estate agent. Please save my contact info card I just sent you! I save my clients 1000's of dollars every deal. No Games! Call or text when ready.", "http://arabfacts.rf.gd/twilio/" . $this->tempfilename, $zip);
        // $this->SendSMS($tonumber, "+14805258426", "Hi, I'm \"K\" Martin a 17yr professional real estate agent. Please save my contact info card I just sent you! I save my clients 1000's of dollars every deal. No Games! Call or text when ready.", "http://arabfacts.rf.gd/twilio/" . $this->tempfilename, $zip);
        //---------------</codenassor>
    }
    ...
    private function add_ringcentral($number, $firstName, $lastName, $email)
    {
        ...
        if ($RingCentral->platform()->loggedIn()) {
            $apiResponse = $RingCentral->platform()->get('/account/~/extension/~/address-book/contact', array(
    ...
    // private function twilio_make_call_old($from, $to, $lead_number)
    // {
    //     $this->twilio_generate_xml($from, $lead_number);
    //
    //     $client = new Twilio\Rest\Client($this->twilio_config["AccountSid"], $this->twilio_config["AuthToken"]);
    //     try {
    //         $call = $client->calls->create($to, $from, array(
    //             "url" => $this->base_domian_path . $this->twilio_forwarder_file
    //         ));
    //         if ($this->debug) {
    //             echo '<div>' . "Started call: " . $call->sid . '</div>';
    //         }
       ...
       <Response>
        ...
        if ($number == 1) {
          ?>
            ...
            echo $this->base_domian_path . $this->twilio_gather_file;
            ?></Redirect>
            <?php
        } else {
          ?>
    ...
    /*public function twilio_gather_handlerold($number)
    {
      ?>
       <Response>
            <?php
        if ($number == 1) {
          ?>
               <Redirect method="POST"><?php
               echo $this->base_domian_path . $this->twilio_forwarder_file;
            ?></Redirect>
            <?php
        } else {
          ?>
    ...
    public function SendVCF($to1, $from1, $body, $vcfurl, $zip)
    {
        ...
        // "+16023631448",client
        // "+16237425423",kyla
        $this->client->messages->create($to1, array(
            "from" => $from1,
            "body" => $body,
            "mediaUrl" => $vcfurl
        ));
    ...
    public function SendVCFkelly($to1, $from1, $body, $vcfurl, $zip)
    {

        $this->makevcfkelly($zip); //die();
        // "+16193978305?",chris
        // "+16023631448",client
        // "+16237425423",kyla
        $this->client->messages->create($to1, array(
    ...
    // public function SendVCFCindy($to1, $from1, $body, $vcfurl, $zip)
    // {
    //     $this->makevcfCindy($zip); //die();
    //     // "+16193978305?",chris
    //     // "+16023631448",client
    //     // "+16237425423",kyla
    //     $this->client->messages->create($to1, array(
    ...
    public function makevcf($zip1)
    {
        ...
        $vcard->addNote("K Martin is known for his professional real estate expertise. With over 18 years of experience as a Realtor and veteran of the US Navy he brings not just his knowledge, but his loyalty to his clients.");
    ...
    public function makevcfkelly($zip1)
    {
        $url        = "www." . $zip1 . "re" . ".com";
        // define vcard
        $vcard      = new VCard();
        // define variables
        $firstname  = 'MY REALTOR'; //MY REALTOR -
        $lastname   = 'Kelly Michaels'; //\"K\" Martin
        $additional = '';
        $prefix     = '';
        $suffix     = '';
        // add personal data
        $vcard->addName($lastname, $firstname, $additional, $prefix, $suffix);
        // add work data
        $vcard->addCompany('Martin & Co. Real Estate');
        //$vcard->addJobtitle('Web Developer');
        $vcard->addEmail('Kmichaelsre@gmail.com');
        //$vcard->addPhoneNumber(1234121212, 'PREF;WORK;');
        $vcard->addPhoneNumber("6023965471", 'WORK');
        #3031, ,
        //$vcard->addAddress("11811 ", 'N Tatum Blvd'," #3031" , 'Phoenix','AZ' , '85028' );
        //$vcard->addURL($url);
        $vcard->addPhoto('re.jpeg');
        ...
        //echo 'A personal vCard is saved in this folder: ' . __DIR__;
        // return vcard as a download
        $vcard->savetolocal($this->tempfilenamekelly);
        // echo message
        // or
        // save the card in file in the current folder
        // return $vcard->save();
    ...
    // public function makevcfCindy($zip1)
    // {
    //     $url        = "www." . $zip1 . "re" . ".com";
    //     // define vcard
    //     $vcard      = new VCard();
    ...
    //     $vcard->addEmail('kmichaelsre@gmail.com');
    //     //$vcard->addPhoneNumber(1234121212, 'PREF;WORK;');
    //     $vcard->addPhoneNumber("6029260047", 'WORK');
    //     #3031, ,
    //     //$vcard->addAddress("11811 ", 'N Tatum Blvd'," #3031" , 'Phoenix','AZ' , '85028' );
    //     //$vcard->addURL($url);
    //     $vcard->addPhoto('re.jpeg');
    ...
    //     //echo 'A personal vCard is saved in this folder: ' . __DIR__;
    //     // return vcard as a download
    //     $vcard->savetolocal($this->tempfilenameCindy);
    //     // echo message
    //     // or
    //     // save the card in file in the current folder
    //     // return $vcard->save();
    ...
    public function saveRecordToDB($data_array)
    {
        $first_name = $data_array['first_name'];
        $last_name  = $data_array['last_name'];
        $email      = $data_array['email'];
        ...
        $mobile     = preg_replace('/[^0-9]/', '', $data_array['mobile']);
        $data       = json_encode($data_array);
        //  $statement = $this->db_con->prepare("INSERT INTO `leads` (`resource_id`, `fname`,`lname`, `email`,`phone`,`data`,`created_date` )
        // VALUES ('666777','$first_name','$last_name', '$email', '$mobile', '$data',NOW())");
        $statement  = $this->db_con->prepare("INSERT INTO leads (`resource_id`, `fname`,`lname`,`phone`,`email`,`data`,`created_date`,`address`,`city`,`state`,`zip`,`mls_id`,`source`,`price`)
                                                         VALUES ('666777','$first_name','$last_name','$mobile',  '$email', '$data',NOW(),'$address','$city','$state','$zip','$mlsId','','$price')");
        var_dump($statement);
        $count = $statement->execute();
        return $this->db_con->lastInsertId();
    }

    public function updateRecord($lid, $agent)
    {
        $statement = $this->db_con->prepare("update `leads` set `Agent`='$agent' where `id`=$lid");
        var_dump($statement);
        $statement->execute();
    }
