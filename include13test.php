...
//$r->SendSMS($tonumber,"+14805258426", "Hi, I'm \"K\" Martin a 17yr professional real estate agent. Please save my contact info card I just sent you! I save my clients 1000's of dollars every deal. No Games! Call or text when ready.", "http://arabfacts.rf.gd/twilio/" . $r->tempfilename, $zip);


    $data_array = array();
    $data_array['first_name'] = "fname";
    $data_array['last_name'] = "lname";
    $data_array['email'] = "hazemonline111@gmail.com";
    $data_array['mobile'] = "+292738738";
    $data_array['address1']="some address";
    $data_array['description'] = "some description";

    $data_array['state'] = "st";
    $data_array['zip'] = 87655;
    $data_array['city'] = "city";
    $data_array['custom_fields']['Inquiry Comment'] = "comment";
    $data_array['custom_fields']['MLSID'] = 43;
    $data_array['custom_fields']['price_of_home'] = 2943;

    $r->saveRecordToDB($data_array);//parsed_data_action($data_array, $r->lead_from[0]);
...
//------------------------------------------------------<added by code nassor>
    ...
    private $tempfilenamekelly = "MyRealtor-kelly.vcf";
    //----------------------------------------------------------</codenassor>
...
//----------------------------------------------------------</codenassor>
    ...
    public function run_email_loop()
    {
        ...
        //sleep(2);        } //end of loop that is done 30 timese
    }
...
//----------------<codenassor>
                    //   $this->testvcard("somezip");//$data_array['zip']);//,$mobile);
...
//---------------</codenassor>
    ...
    public function parsed_data_action($data_array, $lead_from)
    {
        ...
        if ($Arizona < 21 and $Arizona >= 8) {
            if (strpos($data_array['custom_fields']['Inquiry Comment'], 'lease') == false or strpos($data_array['custom_fields']['Inquiry Comment'], 'rent') == false) {
                if ($use_domain_set["app_name"] == "Bravo" && $data_array['zip'] == $lead_from["post_rule_code"]) {
                    ...
                    //$this->twilio_make_call($use_domain_set["from"], $use_domain_set["to"], $data_array['mobile']);
                    if ($property_value > 11) {
                        //$this->testvcardkelly($data_array['zip']);//,$mobile);
                        //  $this->testvcard($data_array['zip']);//,$mobile);
                        sleep(1);
                        $this->twilio_make_call($use_domain_set["from"], $use_domain_set["to"], $data_array['mobile']);
                    } else {
                        //Kelly
...
//deleted kuv
    ...
    //----------------<codenassor>
    public function testvcard($zip,$tonumber="+16023631448")
    {
        //$zip="somezip";
        $currentpath = "http://rdccharlie.com/basecrm/";
        // Your Account Sid and Auth Token from twilio.com/user/accouAnt
        $this->SendVCF($tonumber, "+14805258426", "MY REALTOR - \"K\" Martin", $currentpath . $this->tempfilename, $zip);
    ...
    public function testvcardkelly($zip,$tonumber="+16023631448")
    {
        //$zip="somezip";
        //----------------<codenassor>
        ...
        $currentpath = "http://rdccharlie.com/basecrm/";
// Your Account Sid and Auth Token from twilio.com/user/accouAnt
        $this->SendVCFkelly($tonumber, "+14805258426", "MY REALTOR - Kelly Michaels", $currentpath . $this->tempfilenamekelly, $zip);
        // $this->SendSMS($tonumber, "+14805258426", "Hi, I'm \"K\" Martin a 17yr professional real estate agent. Please save my contact info card I just sent you! I save my clients 1000's of dollars every deal. No Games! Call or text when ready.", "http://arabfacts.rf.gd/twilio/" . $this->tempfilename, $zip);
//---------------</codenassor>
    }

    public function save_to_basecrm($data_array)
    {
...
<Response>
    ...
    public function saveIconZoho($data_array, $rid)
    {
...
// "+16237425423",kyla
        $this->client->messages
            ->create(                $to1,                array(                    "from" =>  $from1,                    "body" => $body,                    "mediaUrl" => $vcfurl                )            );
    }
    //echo "worked";
